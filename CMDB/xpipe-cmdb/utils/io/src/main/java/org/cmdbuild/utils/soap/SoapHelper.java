...
public class SoapHelper {
    ...
    public SoapHelper withBody(String requestBody, Object... args) {
        return this.withBody(format(requestBody, list(args).stream().map(CmStringUtils::toStringOrEmpty).map(StringEscapeUtils::escapeXml10).collect(toList()).toArray(new Object[]{})));
    }
    ...
    public SoapResponse call() {
        ...
        switch (soapVersion) {
            ...
            default ->
                throw new IllegalArgumentException("unsupported soap version = " + soapVersion);
        }
