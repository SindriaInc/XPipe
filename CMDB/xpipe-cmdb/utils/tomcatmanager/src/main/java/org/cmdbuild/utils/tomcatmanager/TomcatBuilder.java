...
//            unpackPostgres();
    ...
    private void unpackToDir() throws IOException, ZipException, InterruptedException {
        ...
        if (isLinux()) {
            String startupDebugScript = IOUtils.toString(getClass().getResourceAsStream("/org/cmdbuild/utils/tomcatmanager/startup_debug.sh"));
...
//				checkArgument(file.getName().endsWith(".war"), "invalid war file = %s", file); TODO validate war file
    ...
    private void runCustomScript() {
        File source = new File(tomcatConfig.getInstallDir(), "webapps/cmdbuild/WEB-INF/lib_ext/postgresql-42.4.1.jar");
        if (source.exists()) {
            File dest = new File(tomcatConfig.getInstallDir(), "lib/postgresql-42.4.1.jar");
            try {
