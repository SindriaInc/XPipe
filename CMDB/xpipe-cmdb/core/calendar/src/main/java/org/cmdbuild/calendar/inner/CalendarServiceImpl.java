...
public class CalendarServiceImpl implements CalendarService {
    ...
    public Email buildNotificationEmail(CalendarEvent event, EmailTemplate notification) {
        ...
        Email email = emailTemplateProcessorService.createEmailFromTemplate(notification, card, map("event", event, "card", card, "emailAddresses", emailAddresses));
    ...
    private CalendarEvent handleTemplateExpressions(CalendarEvent event) {
        if (equal(event.getSource(), CSO_SYSTEM)) {
            ...
            return CalendarEventImpl.copyOf(event)
                    .withDescription(emailTemplateProcessorService.applyEmailTemplateExpr(event.getDescription(), card))
                    .withContent(emailTemplateProcessorService.applyEmailTemplateExpr(event.getContent(), card))
                    .build();
        } else {
            return event;
        }
