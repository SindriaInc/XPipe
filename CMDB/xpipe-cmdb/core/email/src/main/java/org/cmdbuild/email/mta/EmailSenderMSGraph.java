...
public class EmailSenderMSGraph extends BaseEmailSenderProvider {
    ...
    private void addMailDestinations(Message messageMSGraph, Email email) {
        messageMSGraph.toRecipients = email.getToEmailAddressList().stream().map(e -> buildRecipient(e)).collect(toList());
        messageMSGraph.ccRecipients = email.getCcEmailAddressList().stream().map(e -> buildRecipient(e)).collect(toList());
        messageMSGraph.bccRecipients = email.getBccEmailAddressList().stream().map(e -> buildRecipient(e)).collect(toList());
    }
    ...
    private AttachmentCollectionPage buildSmallAttachments(List<EmailAttachment> attachments) {
        ...
        List<Attachment> fileAttachments = attachments.stream().map(a -> buildFileAttachment(a)).collect(Collectors.toList());
