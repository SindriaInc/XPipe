...
public class EmailTemplateServiceImpl implements EmailTemplateService {
    ...
    private final Set<String> fixedMeta = set("showOnClasses", "action", "actionLabel");
    ...
    private EmailTemplate templateFromConfigItem(WaterwayItem item) {
        return EmailTemplateImpl.builder()
                ...
                .withMeta(map(unflattenMap(item.getConfig(), "meta")).accept(m -> {
                    fixedMeta.forEach(meta -> {
                        if (item.hasConfigNotBlank(meta)) {
                            m.put(meta, item.getConfig(meta));
                        }
                    });
                }))
