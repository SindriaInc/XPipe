...
public class BuildInfoUtils {
    ...
    public static BuildInfo parseBuildInfo(Properties properties) {
        try {
            ...
            String versionNumber = checkNotBlank(properties.getProperty("org.cmdbuild.version")),
                    commitInfo = format("%s/%s (%s)%s", commitId, branch, toIsoDateTimeUtc(timestamp), isDirty ? " (dirty)" : ""),
