...
import static org.cmdbuild.plugin.SystemPluginUtils.addPluginFilesToWarFileInplace;
import static org.cmdbuild.plugin.SystemPluginUtils.removePluginsFromWarFileInplace;
import static org.cmdbuild.plugin.SystemPluginUtils.scanFileForPlugin;
...
public class BuildUtilsCommandRunner extends AbstractCommandRunner {
    ...
    protected void configureSystemPlugins(@Nullable String warFileOrWebappDir) throws IOException {
        ...
        Button submitButton = new Button("Apply", unsafe(() -> {
            list(checkBoxList.getItems()).filter(i -> actualPlugins.containsKey(i) && !checkBoxList.isChecked(i)).forEach(toremove::add);
            list(checkBoxList.getItems()).filter(i -> !actualPlugins.containsKey(i) && checkBoxList.isChecked(i)).forEach(toadd::add);
            window.close();
        }));
