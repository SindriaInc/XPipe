...
//        return loadSubreportParamsFromResourcesAndFixReport(jasperDesign);
    ...
    public static javax.activation.DataSource exportReportTemplatesAsZip(ReportDataExt reportData) {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
    ...
    public static Consumer<ReportDataImplBuilder> updateReportData(Map<String, byte[]> files) {
        return (builder) -> {
            ...
            List<String> subreportFileNames = list(subreports).map(ReportUtils::getSubreportName).distinct();
