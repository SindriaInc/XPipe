...
import org.cmdbuild.dms.cmis.CmisConfiguration;
import static org.cmdbuild.dms.cmis.CmisConfiguration.CMIS_PASSWORD;
import static org.cmdbuild.dms.cmis.CmisConfiguration.CMIS_URL;
import static org.cmdbuild.dms.cmis.CmisConfiguration.CMIS_USER;
import static org.cmdbuild.dms.cmis.CmisDmsUtils.getCmisRepository;
import static org.cmdbuild.dms.cmis.CmisDmsUtils.getFolderCreateIfNotExists;
import static org.cmdbuild.dms.inner.DmsProviderService.DMS_PROVIDER_CMIS;
import static org.cmdbuild.dms.inner.DmsProviderService.DMS_PROVIDER_SHAREPOINT_ONLINE;
import org.cmdbuild.dms.sharepoint.SharepointConfiguration;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_CLIENT_ID;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_CLIENT_SECRET;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_PROTOCOL;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_PROTOCOL_DEFAULT;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_RESOURCE_ID;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_SERVICE_URL;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_SERVICE_URL_DEFAULT;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_AUTH_TENANT_ID;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_GRAPH_API_BASE_URL;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_GRAPH_API_BASE_URL_DFAULT;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_PASSWORD;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_URL;
import static org.cmdbuild.dms.sharepoint.SharepointConfiguration.SHAREPOINT_USER;
...
//                        l.add(new EtlHandlerContextImpl(data, map(api.getMeta()).with(FILEREADER_FILENAME_META, FilenameUtils.getName(f))));
            ...
            return switch (parseEnum(api.getConfigNotBlank("source"), DirectorySourceType.class)) {
                ...
                case DS_EXTERNALDMS ->
                    switch (api.getConfigNotBlank("externaldms_type")) {
                        case DMS_PROVIDER_CMIS ->
                            new CmisDirectorySource(new CmisConfigurationImpl(map(api.getConfig()).filterMapKeys("externaldms_cmis_").mapKeys(s -> s.replaceAll("_", "."))));
                        case DMS_PROVIDER_SHAREPOINT_ONLINE ->
                ...
                case DS_INTERNALDMS -> {
                    ...
                    yield switch (dmsConfiguration.getService()) {
                        case DMS_PROVIDER_CMIS ->
                            new CmisDirectorySource(new CmisConfigurationImpl(map(configService.getConfigOrDefaultsAsMap()).filterMapKeys("org.cmdbuild.dms.service.cmis.")));
                        case DMS_PROVIDER_SHAREPOINT_ONLINE ->
        ...
        private class CmisDirectorySource implements DirectorySource {

            private final Session session;

            public CmisDirectorySource(CmisConfiguration config) {
                checkNotNull(config);
                Repository repository = getCmisRepository(config);
            ...
            public List<String> listSourceFiles() {
                ...
                if (recursive) {
                    ...
                    return stream(folder.getChildren()).filter(i -> i instanceof Document).map(Document.class::cast)
                            .filter(f -> (isBlank(excludePattern) || !f.getName().matches(excludePattern)) && (isBlank(filePattern) || f.getName().matches(filePattern)))
                            .map(d -> checkNotBlank(d.getPaths().get(0))).sorted().collect(toImmutableList());
                }
            }

            @Override
            public DataSource loadFile(String file) {
                logger.debug("load cmis file =< {} >", file);
                Document document = (Document) session.getObjectByPath(checkNotBlank(file));
                return newDataSource(toByteArray(document.getContentStream().getStream()), document.getContentStreamMimeType(), document.getContentStreamFileName());
            }
            ...
            public void applyPostImportAction(String file) {
                switch (postImportAction) {
                    case PIA_DELETE_FILES -> {
                        ...
                        ((Document) session.getObjectByPath(checkNotBlank(file))).delete();
                    }
                    case PIA_DISABLE_FILES -> {
                        Document document = (Document) session.getObjectByPath(checkNotBlank(file));
                        String targetForRename = filenameForProcessedFile(document.getName());
                        logger.debug("rename processed file to {}", targetForRename);
                        document.rename(targetForRename);
                    }
                    case PIA_MOVE_FILES -> {
                        logger.debug("move processed file to {}", targetDirectory);
                        Document document = (Document) session.getObjectByPath(checkNotBlank(file));
                        document.move(getOnlyElement(document.getParents()), getFolderCreateIfNotExists(session, targetDirectory));
                    }
                    case PIA_DO_NOTHING -> {
                    }
                    default ->
        ...
        private class CmisConfigurationImpl implements CmisConfiguration {

            private final String url, user, password;

            public CmisConfigurationImpl(Map<String, String> config) {
                this.url = checkNotBlank(config.get(CMIS_URL), "missing cmis `url` param");
                this.user = checkNotBlank(config.get(CMIS_USER), "missing cmis `user` param");
                this.password = decryptValue(checkNotBlank(config.get(CMIS_PASSWORD), "missing cmis `password` param"));
            }
        ...
        private class SharepointConfigurationImpl implements SharepointConfiguration {
            ...
            private final String url, user, password, authProtocol, authResourceId, authClientId, authTenantId, authServiceUrl, authClientSecret, graphApiBaseUrl;
            ...
            public SharepointConfigurationImpl(Map<String, String> config) {
                ...
                this.authProtocol = firstNotBlank(config.get(SHAREPOINT_AUTH_PROTOCOL), SHAREPOINT_AUTH_PROTOCOL_DEFAULT);
                ...
                this.graphApiBaseUrl = firstNotBlank(config.get(SHAREPOINT_GRAPH_API_BASE_URL), SHAREPOINT_GRAPH_API_BASE_URL_DFAULT);
            }
