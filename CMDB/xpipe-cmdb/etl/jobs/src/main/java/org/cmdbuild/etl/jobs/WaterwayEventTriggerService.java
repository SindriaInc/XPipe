    ...
    private class EventHandler {
        ...
        private boolean matchesFilter(WaterwayMessage message) {
            if (filter.isNoop()) {
                ...
                return false;
            } else if (filter.hasFulltextFilter() && !FulltextFilterProcessor.<Map<String, String>>build(filter.getFulltextFilter()).withKeyToValueFunction((k, c) -> c.get(k)).match(serializeMessage(message))) {
                return false;
            } else {
                return true;
            }
        ...
        private boolean matchesFilter(Card card) {
            if (filter.isNoop()) {
                ...
                return false;
            } else if (filter.hasFulltextFilter() && !FulltextFilterProcessor.<Card>build(filter.getFulltextFilter()).withKeyToValueFunction((k, c) -> c.get(k)).match(card)) {
                return false;
            } else {
                return true;
            }
