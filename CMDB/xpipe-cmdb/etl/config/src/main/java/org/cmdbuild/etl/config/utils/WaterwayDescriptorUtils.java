...
public class WaterwayDescriptorUtils {
    ...
    public static List<WaterwayDescriptorRecord> loadSystemDescriptors() {
        return list(new Reflections("org.cmdbuild.etl.waterway.system", new ResourcesScanner()).getResources(Pattern.compile(".*[.]yaml")))
    ...
    private static void validate(JsonNode schema, JsonNode jsonNode) throws ProcessingException {
        ...
        ProcessingReport validate = validator.validate(schema, jsonNode);

        list(validate).filter(p -> p.getLogLevel().compareTo(LogLevel.DEBUG) < 0).forEach(p -> {
            LOGGER.trace("validation {}:\n{}", p.getLogLevel(), toPrettyJson(p.asJson()));
        });
        list(validate).filter(p -> p.getLogLevel().compareTo(LogLevel.WARNING) >= 0).forEach(p -> {
            LOGGER.warn("validation {}:\n{}", p.getLogLevel(), toPrettyJson(p.asJson()));
        });

        LOGGER.debug("validate = {}", validate);
        checkArgument(validate.isSuccess(), "error processing wy config file: %s", list(validate).map(v -> v.toString()).collect(joining(", ")));
    }
