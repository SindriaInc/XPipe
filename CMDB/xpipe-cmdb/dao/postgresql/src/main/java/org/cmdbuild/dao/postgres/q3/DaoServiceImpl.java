...
public class DaoServiceImpl implements DaoService, UserDaoService {
    ...
    public void definitiveDelete(Class model, long cardId) {
        ...
        database.getJdbcTemplate().update(query);
    }
    ...
    public List<CMRelation> getServiceRelationsForCard(CardIdAndClassName sourceCard, CmdbSorter sorter, boolean includeHistory) {
        ...
        if (!sorter.isNoop()) {
            ...
            if (sorter.getElements().size() == 1) {
                query += format(" ORDER BY \"%s\" %s", sorter.getElements().get(0).getProperty(), sorter.getElements().get(0).getDirection());
            } else {
                query += " ORDER BY ";
                for (int i = 0; i < sorter.getElements().size(); i++) {
                    if (i < sorter.getElements().size() - 1) {
                        query += format(" \"%s\" %s AND", sorter.getElements().get(i).getProperty(), sorter.getElements().get(i).getDirection());
                    } else {
                        query += format(" \"%s\" %s", sorter.getElements().get(i).getProperty(), sorter.getElements().get(i).getDirection());
                    }
                }
            }
        }
        if (domains.isEmpty()) {
            ...
            return database.getJdbcTemplate().query(query, (ResultSet rs, int rowNum) -> {
                ...
                return RelationImpl.builder()
                        ...
                        .addAttributes(parseEntryTypeQueryResponseData(domain, getDomainAttributesForRelationQuery(domain).stream().filter(a -> !a.isVirtual()).map(a -> {
                            return switch (a.getType().getName()) {
                                case LOOKUP ->
                                    list(a.getName(), buildCodeAttrName(a.getName()), buildDescAttrName(a.getName()));
                                case LOOKUPARRAY ->
                                    list(a.getName(), buildInfoAttrName(a.getName()));
                                default ->
                                    list(a.getName());
                            };
