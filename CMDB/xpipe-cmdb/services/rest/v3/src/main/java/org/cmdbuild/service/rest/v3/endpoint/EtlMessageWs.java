...
public class EtlMessageWs {
    ...
    public Object readMessages(WsQueryOptions wsQueryOptions) {
        DaoQueryOptions daoQueryOptions = DaoQueryOptionsImpl.copyOf(wsQueryOptions.getQuery()).withAttrs(cardMapperService.getClasseForModelOrBuilder(EtlMessage.class).getCoreAttributes().stream().map(Attribute::getName).filter(a -> !a.equals("Attachments")).collect(toSet())).build(); // skip attachments here
