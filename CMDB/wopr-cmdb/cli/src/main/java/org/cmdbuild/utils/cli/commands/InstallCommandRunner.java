...
public class InstallCommandRunner extends AbstractCommandRunner {
    ...
    protected void exec(CommandLine cmd) throws Exception {
        ...
        System.out.println();
        int tomcatHttpPort = Integer.parseInt(firstNonNull(trimToNull(consoleReader.readLine("tomcat http port <" + TOMCAT_DEFAULT_HTTP_PORT + ">: ")), Integer.toString(TOMCAT_DEFAULT_HTTP_PORT)));
        int tomcatShutdownPort = tomcatHttpPort - TOMCAT_DEFAULT_HTTP_PORT + TOMCAT_DEFAULT_SHUTDOWN_PORT;
        tomcatShutdownPort = Integer.parseInt(firstNonNull(trimToNull(consoleReader.readLine("tomcat shutdown port (offset already applied) <" + tomcatShutdownPort + ">: ")), Integer.toString(tomcatShutdownPort)));
        int tomcatDebugPort = tomcatHttpPort - TOMCAT_DEFAULT_HTTP_PORT + TOMCAT_DEFAULT_DEBUG_PORT;
        tomcatDebugPort = Integer.parseInt(firstNonNull(trimToNull(consoleReader.readLine("tomcat debug port (offset already applied) <" + tomcatDebugPort + ">: ")), Integer.toString(tomcatDebugPort)));
        ...
        while (!hasValidPostgresDatabase) {
            try {
                ...
                String serverVersion = PostgresUtils.newHelper(hostname, Integer.parseInt(port), adminUser, adminPassword).buildHelper().getServerVersion();
