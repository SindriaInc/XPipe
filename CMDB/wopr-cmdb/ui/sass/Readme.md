...
folder are mapped to the application's root namespace, 'CMDBuildUI'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "CMDBuildUI"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'CMDBuildUI' in the same way
as `"CMDBuildUI/sass/src"`.
