Ext.define('CMDBuildUI.view.administration.content.dashboards.card.CardController', {
    ...
    saveDashboardChartLocalizedFieldLocales: function (vm, record) {
        record.charts().each(function (chart) {
            ...
            const idChart = chart.get('_id');
            const idChartReplaced = idChart.replaceAll('-', '_');
            chart.dataSourceParameters().each(function (parameter, index) {
                const parameterDescriptionLocaleVmObject = Ext.String.format('theChartParameterDescription_{0}_{1}', idChartReplaced, index);
                if (vm.get(parameterDescriptionLocaleVmObject)) {
                    const parameterDescriptionTranslationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartParameterName(recordName, idChart, index);
            ...
            const chartDescritpionVmObject = Ext.String.format('theChartDescription_{0}', idChartReplaced);
            if (vm.get(chartDescritpionVmObject)) {
                const chartDescritpionTranslationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartDescription(recordName, idChart);
            ...
            const categoryAxisTitleVmObject = Ext.String.format('theChartCategoryAxisTitle_{0}', idChartReplaced);
            if (vm.get(categoryAxisTitleVmObject)) {
                const categoryAxisTitleTranslationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartCategoryAxis(recordName, idChart);
            ...
            const valueAxisLabelVmObject = Ext.String.format('theChartValueAxisLabel_{0}', idChartReplaced);
            if (vm.get(valueAxisLabelVmObject)) {
                const valueAxisLabelTranslationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartValueAxis(recordName, idChart);
                vm.set(Ext.String.format('{0}._id', valueAxisLabelVmObject), valueAxisLabelTranslationCode);
                vm.get(valueAxisLabelVmObject).save();
            }
            const labelFieldVmObject = Ext.String.format('theChartLabelField_{0}', idChartReplaced);
            if (vm.get(labelFieldVmObject)) {
                const labelFieldTranslationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartLabelField(recordName, idChart);
