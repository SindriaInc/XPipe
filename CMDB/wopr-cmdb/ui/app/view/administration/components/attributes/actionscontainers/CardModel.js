Ext.define('CMDBuildUI.view.administration.components.attributes.actionscontainers.CardModel', {
    ...
    formulas: {
        ...
        domainExtraparamsChained: {
            ...
            get: function (data) {
                var me = this,
                    type = CMDBuildUI.util.helper.ModelHelper.getObjectTypeByName(data.objectTypeName),
                    obj = CMDBuildUI.util.helper.ModelHelper.getObjectFromName(data.objectTypeName);
                if (type && type !== CMDBuildUI.util.helper.ModelHelper.objecttypes.domain) {
                    obj.getAttributes(true).then(function (attributes) {
                        try {
                            var filter = [function (item) {
                                var referencesAttributesDomains = attributes.getRange().filter(function (attr) {
                                    return attr.getId() !== me.get('theAttribute._id') && attr.get('type') === CMDBuildUI.model.Attribute.types.reference && CMDBuildUI.util.helper.ModelHelper.getObjectFromName(attr.get('domain'));
                                }).map(function (attr) {
                                    return CMDBuildUI.util.helper.ModelHelper.getObjectFromName(attr.get('domain'));
                                });
                                var domainAlreadyExistInAttributes = Ext.clone(referencesAttributesDomains).filter(function (i) {
                                    return i.get('name') === item.get('name');
                                });

                                if (('N:N' === item.get('cardinality')) || (('1:N' === item.get('cardinality') || 'N:1' === item.get('cardinality')) && domainAlreadyExistInAttributes.length >= 1) ||
                                    ('1:1' === item.get('cardinality') && domainAlreadyExistInAttributes.length >= 2)) {
                                    return false;
                                }
                            ...
                            me.set('domainExtraparamsChainedFilter', filter);
        ...
        targetClassIsProcess: {
            bind: {
                targetClass: '{theAttribute.targetClass}'
            },
            get: function (data) {
                if (!Ext.isEmpty(data.targetClass)) {
                    var objectType = CMDBuildUI.util.helper.ModelHelper.getObjectTypeByName(data.targetClass);
        ...
        cascadeActions: function () {
            return CMDBuildUI.util.administration.helper.ModelHelper.getCascadeActions().filter(function (a) { return a.value !== 'auto' });
        }
    },
    ...
    stores: {
        domainsStore: {
            ...
            autoDestroy: true,
            filters: '{domainExtraparamsChainedFilter}'
        },
        lookupStore: {
