Ext.define('CMDBuildUI.view.administration.components.viewfilters.card.FormController', {
    ...
    onTranslateClick: function (event, button, eOpts) {
        var me = this;
        var vm = me.getViewModel();
        var className = vm.get('action') === CMDBuildUI.util.administration.helper.FormHelper.formActions.edit ? vm.get('theViewFilter').get('target') : '..';
        var filterName = vm.get('action') === CMDBuildUI.util.administration.helper.FormHelper.formActions.edit ? vm.get('theViewFilter').get('name') : '..';
