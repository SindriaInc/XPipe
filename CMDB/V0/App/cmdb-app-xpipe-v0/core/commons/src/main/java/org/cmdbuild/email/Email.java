...
public interface Email extends EmailCommonData {
    ...
    default boolean isAutoGenerated() {
        List<String> autoGeneratedHeaders = list("auto-submitted: auto-replied", "auto-submitted: auto-replied", "auto-submitted: auto-generated");
        return !list(getHeaders().split("\n")).stream().filter(h -> autoGeneratedHeaders.contains(h.toLowerCase())).toList().isEmpty();
    }
