...
public class EmailWsHelper {
    ...
    public Object createEmail(String classId, String cardId, WsEmailData emailData, Set<EmailTemplateSerializationType> templateSerializationType, List<EmailAttachment> emailAttachments) {
        Card card;
        if (equal(cardId, "_ANY")) {
            card = emailData.hasCardData() ? CardImpl.buildCard(dao.getClasse(equal(classId, "_ANY") ? toStringNotBlank(emailData.getCardData().getValues().get("_type")) : checkNotBlank(classId)), emailData.getCardData().getValues()) : null;
        } else {
