    ...
    private List<Pair<Map<String, Object>, Map<String, Object>>> applyFilter(List<Pair<Map<String, Object>, Map<String, Object>>> standardDetailedList) {
        if (filter.isNoop()) {
            ...
            if (filter.hasFulltextFilter()) {
                standardDetailedList = FulltextFilterProcessor.<Pair<Map<String, Object>, Map<String, Object>>>build(filter.getFulltextFilter()).withKeyFunction(r -> r.getRight().keySet()).withKeyToValueFunction((k, r) -> r.getRight().get(k)).filter(standardDetailedList);
