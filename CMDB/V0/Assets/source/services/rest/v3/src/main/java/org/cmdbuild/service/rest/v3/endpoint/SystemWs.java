...
import org.cmdbuild.service.rest.v3.helpers.SystemPluginHelper;
...
//            configService.reload(); TODO check this
    ...
    public Object getSystemPlugins() {
        return response(list(pluginService.getSystemPlugins()).map(pluginHelper::serializePlugin));
    }

    @GET
    @Path("plugins/{pluginCode}")
    public Object getSystemPlugin(@PathParam("pluginCode") String pluginCode) {
    ...
    @POST
    @Path("plugins/{pluginCode}/patch")
    public Object applySystemPluginsPatches(@PathParam("pluginCode") String pluginCode) {
