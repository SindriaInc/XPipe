Ext.define('CMDBuildUI.view.administration.content.dashboards.card.chart.FormController', {
    ...
    onTranslateChartDescriptionClick: function (event, button, eOpts) {
        ...
        const ownerVm = vm.get('owner').getViewModel();
        const chartId = vm.get('theChart._id');
        const translationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartDescription(ownerVm.get('theDashboard.name'), chartId);
        const vmLocaleObject = Ext.String.format('theChartDescription_{0}', chartId.replaceAll('-', '_'));
        CMDBuildUI.util.administration.helper.FormHelper.openLocalizationPopup(translationCode, vm.get('action'), vmLocaleObject, ownerVm, true);
    },
    ...
    onTranslateCategoryAxisTitleClick: function (event, button, eOpts) {
        ...
        const ownerVm = vm.get('owner').getViewModel();
        const chartId = vm.get('theChart._id');
        const translationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartCategoryAxis(ownerVm.get('theDashboard.name'), chartId);
        const vmLocaleObject = Ext.String.format('theChartCategoryAxisTitle_{0}', chartId.replaceAll('-', '_'));
        CMDBuildUI.util.administration.helper.FormHelper.openLocalizationPopup(translationCode, vm.get('action'), vmLocaleObject, ownerVm, true);
    },
    ...
    onTranslateValueAxisLabelClick: function (event, button, eOpts) {
        ...
        const ownerVm = vm.get('owner').getViewModel();
        const chartId = vm.get('theChart._id');
        const translationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartValueAxis(ownerVm.get('theDashboard.name'), chartId);
        const vmLocaleObject = Ext.String.format('theChartValueAxisLabel_{0}', chartId.replaceAll('-', '_'));
        CMDBuildUI.util.administration.helper.FormHelper.openLocalizationPopup(translationCode, vm.get('action'), vmLocaleObject, ownerVm, true);
    },
    ...
    onTranslateChartLabelFieldClick: function (event, button, eOpts) {
        ...
        const ownerVm = vm.get('owner').getViewModel();
        const chartId = vm.get('theChart._id');
        const translationCode = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartLabelField(ownerVm.get('theDashboard.name'), chartId);
        const vmLocaleObject = Ext.String.format('theChartLabelField_{0}', chartId.replaceAll('-', '_'));
        CMDBuildUI.util.administration.helper.FormHelper.openLocalizationPopup(translationCode, vm.get('action'), vmLocaleObject, ownerVm, true);
    },
    ...
    privates: {
        ...
        undoLocales: function (theDashboard, theChart, vm) {
            ...
            const chartId = theChart.get('_id').replaceAll('-', '_');

            const chartDescriptionVmObject = Ext.String.format('theChartDescription_{0}', chartId);
            if (vm.get(chartDescriptionVmObject)) {
                vm.set(chartDescriptionVmObject, undefined);
            }

            const categoryAxisTitleVmObject = Ext.String.format('theChartCategoryAxisTitle_{0}', chartId);
            if (vm.get(categoryAxisTitleVmObject)) {
                vm.set(categoryAxisTitleVmObject, undefined);
            }

            const valueAxisLabelVmObject = Ext.String.format('theChartValueAxisLabel_{0}', chartId);
            if (vm.get(valueAxisLabelVmObject)) {
                vm.set(valueAxisLabelVmObject, undefined);
            }

            const labelFieldVmObject = Ext.String.format('theChartLabelField_{0}', chartId);
        ...
        undoParametersLocalizations: function (theDashboard, theChart, parameters, vm, forceDeletion) {
            Ext.Array.forEach(parameters.items, function (parameter, index) {
                const parameterDescriptionLocaleVmObject = Ext.String.format('theChartParameterDescription_{0}_{1}', theChart.get('_id').replaceAll('-', '_'), index);
                if (forceDeletion) {
                    CMDBuildUI.util.Ajax.setActionId('delete-chartparameter-locales');
                    const localeId = CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartParameterName(theDashboard.get('name'), theChart.get('_id'), index);
                    Ext.Ajax.request({
