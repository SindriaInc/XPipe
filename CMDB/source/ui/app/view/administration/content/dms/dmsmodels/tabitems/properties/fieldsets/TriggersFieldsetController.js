Ext.define('CMDBuildUI.view.administration.content.dms.models.tabitems.properties.fieldsets.TriggersFieldsetController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.administration-content-dms-models-tabitems-properties-fieldsets-triggersfieldset',
    ...
    onAddNewTriggerBtn: function (grid, rowIndex, colIndex) {
        ...
        var cleanRecord = CMDBuildUI.util.administration.helper.ModelHelper.setReadState(Ext.create('CMDBuildUI.model.FormTrigger', {
            script: ''
        }));
        newTriggerStore.add(cleanRecord);
        grid.refresh();
    ...
    formTriggerCheckChange: function (element, newValue, oldValue, eOpts) {        
        element.up('administration-content-dms-models-tabitems-properties-properties').lookupViewModel().get('formTriggersStoreNew').getData().items[0].set(element.recordKey, newValue);
    },

    onEditBtn: function (button, rowIndex, colIndex) {
        var vm = button.up('administration-content-dms-models-tabitems-properties-properties').lookupViewModel();
