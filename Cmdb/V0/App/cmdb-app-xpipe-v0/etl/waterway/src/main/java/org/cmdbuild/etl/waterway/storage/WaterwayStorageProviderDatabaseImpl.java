...
public class WaterwayStorageProviderDatabaseImpl implements WaterwayStorageProvider {
    ...
    public List<WaterwayStorageHandler> getStorageHandlers() {
        ...
        List<WaterwayStorageHandler> list = (List) config.getAllItems().stream().filter(i -> i.isOfType(WYCIT_STORAGE) && equal(firstNotBlank(i.getConfig(WY_STORAGE_TYPE), DATABASE_STORAGE_PROVIDER_TYPE), DATABASE_STORAGE_PROVIDER_TYPE)).map(i -> new DatabaseStorageProvider(i)).collect(toImmutableList());
    ...
    public PagedElements<WaterwayMessage> getMessages(DaoQueryOptions query) {
        List<WaterwayMessage> messages = list(
                (query.hasAttrs() ? dao.select(query.getAttrs()) : dao.selectAll())
                        .from(EtlMessage.class).withOptions(query.mapAttrNames(WATERWAY_MESSAGE_ATTR_MAPPING)).asList(EtlMessage.class)
        ).map(this::toMessage);
        long total;
