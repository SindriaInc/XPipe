...
public class GrantServiceImpl implements GrantService {
    ...
    public GrantServiceImpl(DashboardRepository dashboardRepository, EtlGateRepository etlGateRepository, EtlTemplateRepository importExportTemplateService, EventBusService grantEventBus, ReportService reportService, GrantDataRepository repository, DaoService dao, ViewDefinitionService viewService, CardFilterService filterStore, CustomPageService customPagesService, CacheService cacheService) {
    ...
    public List<GrantData> getGrantsForRoleIncludeRecordsWithoutGrant(long roleId) {
        ...
        etlGateRepository.getAll().stream().filter((v) -> !grantsForRoleKeys.contains(key(POT_ETLGATE, v.getCode()))).map((v) -> builder.get().withType(POT_ETLGATE).withObjectCode(v.getCode()).build()).forEach(list::add);
    ...
    @Override
    public Collection<Grant> getDashboardPrivilegesByGroupId(long groupId) {
    ...
    public GrantData getGrantDataByRoleAndTypeAndName(Long id, PrivilegedObjectType objectType, String objectTypeName) {
        ...
        return grantData.stream().filter(p -> p.getType().equals(objectType) && toStringOrNull(p.getObjectIdOrClassNameOrCode()).equals(objectTypeName)).findFirst()
                .orElse(grantData.stream().filter(p -> {
