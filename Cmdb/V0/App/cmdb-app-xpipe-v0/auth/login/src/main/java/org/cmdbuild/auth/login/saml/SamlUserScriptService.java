 ...
 */
@Component
public class SamlUserScriptService implements UserScriptService {
    ...
    public SamlUserScriptService(GroovyScriptService scriptService, CmApiService apiService) {
    ...
    public LoginUserIdentity getLoginFromScript(LoginModuleConfiguration authConf, Logger customLogger, Object loginData) {
        try {
            ...
            Map<String, Object> out = scriptService.executeScript(authConf.getLoginHandlerScript(), map(
                    ...
                    "logger", customLogger,
                    "auth", (SamlAuthenticator.AuthResponse) loginData,
                    "login", null
            ));
            return LoginUserIdentity.build(toStringNotBlank(out.get("login"), "invalid login output from saml response handler script"));
        } catch (Exception ex) {
