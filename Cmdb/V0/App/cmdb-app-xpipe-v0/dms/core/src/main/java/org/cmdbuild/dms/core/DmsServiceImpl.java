...
public class DmsServiceImpl implements DmsService, MinionComponent {
    ...
    public DmsServiceImpl(
            UserCardHelperService userCardService,
            List<DmsProviderService> services,
            ...
            VirtualAttributeService virualAttributeService,
        ...
        if (!queryOptions.getFilter().isNoop()) {
            ...
            if (commonDmsModel.getRight()) {
                dao.queryFromSuperclass(commonDmsModel.getLeft()).enableSmartSubclassFilterProcessing().enableAllSubclassesProcessing()
                        .withOptions(DaoQueryOptionsImpl.copyOf(queryOptions).withSorter((CmdbSorter) null).build()).getCards().forEach(c -> matchingAttributeFilterIds.add(c.getString(DOCUMENT_ATTR_DOCUMENTID)));
            } else {
            ...
            if (attachmentWithFilenameAlreadyExists) {
                switch (existingAction) {
                    ...
                    case OEA_REPLACE:
                        ...
                        return updateDocumentWithFilename(className, cardId, documentData.getFilename(), documentData);
                    case OEA_FAIL:
                    default:
