...
public class EmailProviderStrategy {
    ...
    public EmailSenderProvider buildSender(EmailAccount emailAccount,
                                           EmailQueueConfiguration queueConfig,
        ...
        return switch (emailAccount.getAuthenticationType()) {
            case AUTHENTICATION_TYPE_MS_OAUTH2 ->
                new EmailSenderMSGraph(emailAccount,
                                       emailMSGraphClientStrategy.buildMSGraphClientProvider(
                                       emailAccount),
                                       infoService,
                                       signatureService);
            case AUTHENTICATION_TYPE_GOOGLE_OAUTH2 ->
                new EmailSenderGoogle(emailAccount, queueConfig, infoService,
                                      signatureService);
            case AUTHENTICATION_TYPE_DEFAULT ->
                new EmailSenderJavaMail(emailAccount, queueConfig, infoService,
                                        signatureService);
            default ->
                throw new EmailException(format(
                        "sender unsupported authentication type =< %s >",
                        emailAccount.getAuthenticationType()));
        };
    }

    public EmailReceiverProvider buildReceiver(EmailAccount emailAccount,
        ...
        return switch (emailAccount.getAuthenticationType()) {
            case AUTHENTICATION_TYPE_MS_OAUTH2 ->
                new EmailReceiverMSGraph(emailAccount,
                                         emailMSGraphClientStrategy.buildMSGraphClientProvider(
                                         emailAccount), receiveConfig,
                                         lockService, repository);
            case AUTHENTICATION_TYPE_GOOGLE_OAUTH2 ->
                new EmailReceiverGoogle(emailAccount, receiveConfig, lockService,
                                        repository);
            case AUTHENTICATION_TYPE_DEFAULT ->
                new EmailReceiverJavaMail(emailAccount, receiveConfig,
                                          lockService, repository);
            default ->
                throw new EmailException(format(
    ...
    public EmailImapSessionProvider buildImapSessionProvider(
            EmailAccount emailAccount, Logger logger) {
        return switch (emailAccount.getAuthenticationType()) {
            case AUTHENTICATION_TYPE_GOOGLE_OAUTH2:
            ...
            default:
                throw new EmailException(format(
                        "imap session provider unsupported authentication type =< %s >",
                        emailAccount.getAuthenticationType()));
        };
    }
    ...
    public EmailSmtpSessionProvider buildSmtpSessionProvider(
        ...
        return switch (emailAccount.getAuthenticationType()) {
            case AUTHENTICATION_TYPE_GOOGLE_OAUTH2:
                yield new EmailSmtpSessionGoogleProvider(emailAccount,
                                                         smtpTimeoutSeconds,
                                                         logger);
            case AUTHENTICATION_TYPE_DEFAULT:
                yield new EmailSmtpSessionJavaMailProvider(emailAccount,
                                                           smtpTimeoutSeconds,
                                                           logger);
            case AUTHENTICATION_TYPE_MS_OAUTH2:
            default:
