...
public class JoinViewQueryServiceImpl implements JoinViewQueryService {
    ...
    private class JoinViewQueryHelper {
        ...
        private void checkUserViewPermissions() {
            if (applyPrivileges) {
                list(config.getJoinElements()).map(JoinElement::getTargetAlias).with(config.getMasterClassAlias()).map(t -> (EntryType) getTableByAlias(t)).filter(EntryType::isClasse).map(EntryType::asClasse).forEach(c -> {
                    checkArgument(userClassService.getUserClass(c.getName()).hasServiceReadPermission(), "CM: user read access denied for classe = %s", c);
                });
            }
        }
        ...
        private class QueryHelper {
            ...
            private PagedElements<Card> getCards(DaoQueryOptions queryOptions) {
                ...
                config.getJoinElements().forEach(j -> {
                    ...
                    JoinTable source = join.stream().filter(sj -> equal(sj.getAlias(), aliasBuilder.getAlias(TABLE_ALIAS, j.getSource()))).collect(onlyElement());
                ...
                list(join).filter(j -> j.getTable().isClasse() && j.hasSuperclassHelper()).forEach(j -> {
                    j.getSuperclassHelper().getSubclassesUserCardAccess().values().forEach(a -> {
                        a.getFilterMarkNames().forEach(f -> {
//                            String key =  TODO
//                            attributes.add("%s.%s %s".formatted(j.getAlias(),));
                        });
                    });
//                    j.getSuperclassHelper().getSubclassesUserCardAccess().//TODO aggiungere anche filtro su righe - passare avanti i marker (!!)
                });
                ...
                String query = format("SELECT %s FROM %s", Joiner.on(", ").join(attributes), join.stream().map(j -> {
                    String tableExpr;
//                    if (j.getTable().isClasse() && j.getTable().asClasse().isSuperclass()) {
                    if (j.hasSuperclassHelper()) {
//                        tableExpr = "( %s )".formatted(list(accessService.getSuperclassQueryHelper(j.getTable().asClasse()).getSubclassesUserCardAccess()).map(sub
//                                -> "SELECT * FROM %s WHERE \"Status\" = 'A'".formatted(entryTypeToSqlExpr(sub.getUserClass()))).collect(joining(" UNION ALL ")));
                        ...
                        userJoinAttributes.forEach(rethrowConsumer(a -> {
                            ...
                            if (applyPrivileges) {
                                ...
                                if (j.hasSuperclassHelper()) {
                                    ...
                                    Attribute userAttr = cardAccess.getUserClass(cardAccess.getActiveFilters(unsafeFunction(k
                                            -> r.getObject("TODO") //TODO user access mark
                                    ))).getAttributeOrNull(attribute.getName());
