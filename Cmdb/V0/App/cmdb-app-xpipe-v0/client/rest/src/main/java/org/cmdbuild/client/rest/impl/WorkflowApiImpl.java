...
public class WorkflowApiImpl extends AbstractServiceClientImpl implements WokflowApi {
    ...
    @Override
    public void migrateProcess(String processId, FlowMigrationConfig config) {
        checkNotBlank(processId, "process id cannot be null");
        checkNotNull(config, "config param cannot be null");
        post(format("processes/%s/migration", encodeUrlPath(processId)), map().accept(m -> {
            if (config.getDefaultXpdl() != null) {
                m.put("default", config.getDefaultXpdl().isNew() ? map("type", "new", "content", readToString(config.getDefaultXpdl().getContent())) : map("type", "legacy", "target", config.getDefaultXpdl().getPlanId()));
            }
            m.put("mapping", config.getFlowMigrationMapping().entrySet().stream().map(e -> map(
                    "source", e.getKey(),
                    "target", e.getValue().getPlanId(),
                    "type", serializeEnum(e.getValue().getType()),
                    "content", e.getValue().isNew() ? readToString(e.getValue().getContent()) : null
            )).collect(toList()));
        }));
    }
