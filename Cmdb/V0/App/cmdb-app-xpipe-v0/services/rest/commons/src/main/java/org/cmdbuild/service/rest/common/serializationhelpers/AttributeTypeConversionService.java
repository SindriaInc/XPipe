            ...
            attribute.getType().accept(new NullAttributeTypeVisitor() {
                    ...
                    map.put("domain", domain.getName(),
                            ...
                            .accept(m -> {
                                if (attribute.hasFilter()) {
                                    m.put("filter", attribute.getFilter());
                                }

                                if (attribute.hasFilter() || attribute.getMetadata().isUseDomainFilter()) {
                                    m.put("useDomainFilter", attribute.getMetadata().isUseDomainFilter());

                                    final String referencedClassToken = domain.getReferencedClassToken(attribute);
                                    final String ecqlId = domain.getMetadata().getReferenceFilters().get(EcqlUtils.buildUniqueClassToken(referencedClassToken, attribute.getOwnerClass()));
                                    if (isNotBlank(ecqlId)) { // ecqlId is blank while  *creating* a Class Atrribute with filter from ClassAttributeWs: skip the following
                                        final EcqlBindingInfo ecqlBindingInfo = EcqlUtils.getEcqlBindingInfoForExpr(ecqlRepository.getById(ecqlId));
                                        m.put("_referenceFilters",
                                                EcqlFilterSerializationHelper.buildEcqlFilterStuff(
                                                        referencedClassToken, ecqlId,
                                                        referencedClassToken + "_ecqlFilter", ecqlId,
                                                        ecqlBindingInfo)
                                        );
                                    }
                                } // end if
                ...
                private void attachEcqlFilterStuffIfApplicable() {
                    if (attribute.hasFilter()) {
                        ...
                        String ecqlId;
                        if (attribute.getOwner().isReport()) {
                            ecqlId = EcqlUtils.buildReportAttrEcqlId(attribute);
