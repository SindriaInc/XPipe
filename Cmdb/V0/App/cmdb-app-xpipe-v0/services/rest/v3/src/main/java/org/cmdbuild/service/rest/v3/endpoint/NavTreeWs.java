...
public class NavTreeWs {
            ...
            list = AttributeFilterProcessor.<NavTree>builder()
                    .withKeyToValueFunction((key, tree) -> {
                        switch (checkNotBlank(key)) {
                            ...
                            case "type":
                                return serializeEnum(tree.getType());
                            default:
    ...
    private FluentMap serializeTree(NavTree root, TreeMode mode) {
        ...
        switch (mode) {
            case FLAT:
                nodes = root.getData().getThisNodeAndAllDescendants().stream().map(n -> serializeNode(root, n)).collect(toList());
                break;
            case TREE:
                nodes = singletonList(serializeNodeAndDescendants(root, root.getData()));
                break;
            default:
    ...
    public static class WsTreeNodeData {
        ...
        public WsTreeNodeData(Map<String, Object> values) {
            ...
            this.nodes = fromJson(toJson(values.get("nodes")), new TypeReference<List<WsTreeNodeData>>() {
            }.getType());
