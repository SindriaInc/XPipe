...
public final class DatabaseCreatorConfigImpl implements DatabaseCreatorConfig {
    ...
    private final boolean useLimited, createDatabase, keepLocalConfig, installPostgres, skipPatches, vacuumAnalyzeAfterImport;
    ...
    public static class DatabaseCreatorConfigImplBuilder implements Builder<DatabaseCreatorConfig> {
        ...
        public DatabaseCreatorConfigImplBuilder withConfig(InputStream inputStream) {
            return this.withConfig(map(loadProperties(inputStream)).mapValues((k, v) -> Cm3EasyCryptoUtils.decryptValue(v)));
        }

        public DatabaseCreatorConfigImplBuilder withConfig(File configFile) {
            return this.withConfig(map(loadProperties(configFile)).mapValues((k, v) -> Cm3EasyCryptoUtils.decryptValue(v)));
        }
