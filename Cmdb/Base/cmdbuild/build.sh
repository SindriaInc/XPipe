...
if [[ -z "$3" ]]; then
    echo "Provide a tag suffix as third argument (eg. 1.17.2)"
    exit 1
fi

if [[ -z "$4" ]]; then
    echo "Provide a tomcat version as fourth argument (eg. 8.5.93)"
    exit 1
fi

if [[ -z "$5" ]]; then
    echo "Provide a java version as fifth argument (eg. 17)"
    exit 1
fi

if [[ -z "$6" ]]; then
    echo "Provide a tag arch as sixth argument (eg. amd64, arm64)"
    exit 1
fi
